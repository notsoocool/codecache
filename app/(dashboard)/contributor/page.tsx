...
const ContributorCard: React.FC<Contributor> = ({ login, avatar_url, html_url, contributions, type }) => (
  ...
  >
    <div className="p-6 text-center">
      <img src={avatar_url} alt={login} className="w-24 h-24 rounded-full mx-auto mb-4 border-4 border-gray-200 dark:border-gray-600" />
      ...
      <div className="mt-4 bg-gray-100 dark:bg-gray-700 rounded-full py-2 px-4 inline-block">
        <span className="font-semibold text-gray-800 dark:text-white">{contributions} contributions</span>
      </div>
    </div>
    <div className="bg-gray-50 dark:bg-gray-700 py-3 px-6 flex justify-between items-center">
      <a 
        href={html_url} 
        target="_blank" 
        rel="noopener noreferrer" 
        className="text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white transition-colors flex items-center"
      >
...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon }) => (
  <motion.div 
    ...
    className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 flex items-center border border-gray-200 dark:border-gray-700"
  >
...
export default function Contributor() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const contributorsResponse = await fetch('https://api.github.com/repos/AkshitLakhera/PenCraft-Full-Stack-Blogging-Application/contributors')
        const contributorsData: Contributor[] = await contributorsResponse.json()
        setContributors(contributorsData)

        const repoResponse = await fetch('https://api.github.com/repos/AkshitLakhera/PenCraft-Full-Stack-Blogging-Application')
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <StatCard 
              ...
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-gray-600 dark:text-gray-400" viewBox="0 0 20 20" fill="currentColor">
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-gray-100 dark:bg-gray-800">
        <div  className="max-w-7xl mx-auto">
          <h2 className="text-4xl font-bold text-center mb-12 text-gray-900 dark:text-white">Meet Our Contributors</h2>
          <AnimatePresence>
            {loading ? (
              <motion.div
                initial={{ opacity: 0 }}
      ...
      <section id="contribute" className="py-16 px-4 sm:px-6 lg:px-8 bg-gray-900 text-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl mb-8 text-gray-300">
            Join our community and help shape the future of PenCraft.
          </p>
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 justify-center">
            <Input
              ...
              className="bg-white text-gray-800 dark:bg-gray-700 dark:text-white w-full sm:w-auto"
            />
            <Button type="submit" variant="secondary" size="lg" className="bg-white text-gray-900 hover:bg-gray-100 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600">
              Get Started
            </Button>
          </form>
          {notification.show && (
