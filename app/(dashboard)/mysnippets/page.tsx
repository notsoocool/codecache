...
export default function Bookmarks() {
  ...
  const fetchUsersSnippets = async (userId: any) => {
    try {
      const response = await fetch(`/api/mysnippets/${userId}`, {
        method: "GET",
      });
  ...
  useEffect(() => {
    const fetchCurrentUserAndSnippets = async () => {
      setLoading(true);
      try {
        ...
        await fetchUsersSnippets(userData.id);
      } catch (error) {
